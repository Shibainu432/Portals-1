...
impl Renderer {
    pub fn new(
        canvas: &web_sys::HtmlCanvasElement,
        trefoil_geometry: impl IntoIterator<Item = crate::modeling::Triangle>,
        ...
        let mut data: Vec<f32> = trefoil_geometry
            .into_iter()
            .flat_map(triangle_to_array)
            .collect::<Vec<f32>>();
        let num_verts_trefoil = data.len() / 35;
        ...
        let num_verts_static = data.len() / 35 - num_verts_trefoil;
    ...
    pub fn render(&self, uniforms: Uniforms, mut dynamic_geometry: Vec<crate::modeling::Triangle>) {
        ...
        self.gl.uniform1f(
            self.gl.get_uniform_location(&self.program, "is_warped").as_ref(),
            0.0,
        );
        self.gl
            .draw_arrays(GL::TRIANGLES, 0, self.num_verts_trefoil as i32);
        ...
        self.gl
            .draw_arrays(GL::TRIANGLES, self.num_verts_trefoil as i32, self.num_verts_static as i32);
...
const VERTEX_SHADER_SOURCE: &str = concat!(
    include_str!("shaders/vertex_prelude.glsl"),
