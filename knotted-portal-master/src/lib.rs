...
impl Model {
    fn new() -> Self {
        ...
        let trefoil_geometry = modeling::trefoil();
        let static_geometry = modeling::skybox()
            .into_iter()
            .chain(modeling::ground())
            .chain(modeling::environment());
        ...
        Self {
            ...
            fps: None,
            keys: HashSet::new(),
            renderer: render::Renderer::new(&canvas, trefoil_geometry, static_geometry),
